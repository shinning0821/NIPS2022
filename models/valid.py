...
def compute_F1Metric(args,device,val_loader,model):
    f1_metric = torchmetrics.F1(
            num_classes=args.num_class, average='macro', ignore_index=0, mdmc_average='global', multiclass=True)
    val_images = None
    val_labels = None
    val_outputs = None
    for val_data in val_loader:
        ...
        # compute F1metric for current iteration
        f1_metric(val_outputs,val_labels_onehot[:])
    # aggregate the final mean dice result
    metric = f1_metric.compute()
    f1_metric.reset()
    return metric,metric,val_images,val_labels,val_outputs
